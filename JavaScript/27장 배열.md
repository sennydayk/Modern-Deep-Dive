...
- 배열이란, 여러 개의 값을 순차적으로 나열한 자료구조를 의미
...
- **인덱스 (Index) :** 배열에서 배열의 요소의 위치 (0 이상의 정수)
- **접근법** : 대괄호 표기법을 통해 요소에 접근할 수 있다.
...
- **생성** : 배열 리터럴, Array 생성자 함수, Array.of, Array.from 메서드로 생성이 가능
...
- **밀집 배열 (dense array)**
    - 배열의 요소가 하나의 데이터 타입으로 통일되어 있고 연속적으로 인접한 배열
...
- 유사 배열 객체 또는 이터러블 객체를 인수로 전달받아 배열을 생성한다.
...
- 빈 배열이면 `undefined`를 반환하며, 원본 배열을 직접 변경한다.
...
- 값 또는 배열을 인수로 전달받아 원본 배열의 마지막 요소로 추가한 새로운 배열을 반환한다.
...
- `indexOf` 로 특정 요소의 인덱스를 알아내 제거할 수 있다.
...
- 인수로 전달받은 값으로 배열의 처음부터 끝까지 변경한다.
- ES6에서부터 도입되었다.
...
- **고차 함수 (Higher-Order Function, HOF)**
    ...
    외부 상태의 변경이나 가변 데이터를 피하고, 불변성을 지향하는 함수형 프로그래밍에 기반한다.
...
- `sort`와 `reverse`를 사용하여 내림차순 정렬 가능.
...
- ES6의 화살표 함수와 같이 사용하면 `this`의 바인딩 이슈도 깔끔하게 해결하면서 사용할 수 있다.
...
- `forEach`문은 함수이기 때문에 `return` 으로 탈출할 수 있다.
...
- 자신을 호출한 배열을 순회하면서 콜백 함수의 반환값이 `true`인 요소로만 구성된 새로운 배열을 반환한다.
...
- `map`과 달리 `filter`가 반환한 새로운 배열은 기존의 배열과 길이가 다를 수 있다.

### 27.9.5 Array.prototype.reduce

- 자신을 호출한 배열을 순회하면서 콜백 함수를 호출한다.
- 콜백 함수의 반환값은 다음 순회에서의 콜백 함수의 첫 번째 인수로 전달된다.
- 이런 식으로 순회하면서 최종적으로는 하나의 결과값을 반환한다.
...
- 자신을 호출한 배열을 순회하면서 콜백 함수를 호출한다.
- 콜백 함수 반환값이 모두 참이면 `true`, 거짓이면 `false`를 반환한다.
...
- 자신을 호출한 배열을 순회하면서 콜백 함수를 호출하면서 반환값이 `true`인 첫 번째 요소를 반환한다.
...
- 자신을 호출한 배열을 순회하면서 콜백 함수를 호출하면서 반환값이 `true`인 첫 번째 요소의 인덱스를 반환한다.
